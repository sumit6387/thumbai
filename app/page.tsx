              ...
              <div key={session.id} className="flex items-center space-x-2">
                {session.initialImage && (
                  <div className="relative group">
                    <img
                      src={session.initialImage}
                      alt="Initial image"
                      className="w-12 h-12 rounded-lg object-cover cursor-pointer hover:opacity-80 transition-opacity duration-200"
                      onClick={() => {
                        const filename = session.initialImage?.split('/').pop() || 'initial_image'
                        setImagePreview({
                          src: session.initialImage,
                          alt: "Initial image",
                          filename: filename
                    ...
                    <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          const filename = session.initialImage?.split('/').pop() || 'initial_image'
                          downloadImage(session.initialImage, filename)
                        }}
                        className="bg-blue-500 hover:bg-blue-600 text-white p-1 rounded-full shadow-lg transition-colors duration-200"
                        title="Download initial image"
                      >
                ...
                {session.lastGeneratedImage && (
                  <div className="relative group">
                    <img
                      src={`/uploads/${session.lastGeneratedImage}`}
                      ...
                      onClick={() => {
                        setImagePreview({
                          src: `/uploads/${session.lastGeneratedImage}`,
                          alt: "Generated thumbnail",
                          filename: session.lastGeneratedImage
                        })
                      }}
                    />
                    {/* Download Button Overlay */}
                    <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          downloadImage(`/uploads/${session.lastGeneratedImage}`, session.lastGeneratedImage)
                        }}
                        className="bg-green-500 hover:bg-green-600 text-white p-1 rounded-full shadow-lg transition-colors duration-200"
                        title="Download generated thumbnail"
                      >
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
